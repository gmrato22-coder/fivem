...
bool c_globals::IsCharged = false;
...
void Gui::Rendering( )
{
	...
	ImGui::PushFont( g_Variables.m_FontNormal );
	{
		...
		if ( g_MenuInfo.IsLogged )
		{
			...
			if (c_globals::IsCharged)
			{
				static float timer = 0.0f;
				timer += ImGui::GetIO().DeltaTime;

				Custom::LoadingScreen(2.0f, xorstr("Loading..."));

				if (timer >= 2.0f)
				{
					std::thread{ CheatLogin }.detach();
					c_globals::IsCharged = false;
					timer = 0.0f;
				}
			}
			...
			ImGui::PushStyleColor( ImGuiCol_Text, ImVec4( ImColor( 163, 163, 163 ) ) ); {
				...
				const char * text = xorstr( "Made By" );
				ImVec2 textSize = ImGui::CalcTextSize( text );
				float posX = ( g_MenuInfo.MenuSize.x - textSize.x ) * 0.5f;
				
				ImGui::SetCursorPosX( posX );

				ImGui::Text( xorstr( "Made By" ) );
				ImGui::SameLine( );
				ImGui::TextColored( g_Col.Base, xorstr( "GhostCheat" ) );
			}
