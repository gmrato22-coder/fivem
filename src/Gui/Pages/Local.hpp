		...
		ImGui::BeginGroup( );
		{
			...
			case 0: //Main
				...
				ImGui::BeginChild(xorstr("Misc"), ImVec2(230, 400), false, 0);
				{
					...
					ImGui::Keybind(xorstr("Heal Key"), &g_Config.Player->KeyBind, &KeyMode);

					// Überprüfe, ob die gebundene Taste gedrückt wird
					if (GetAsyncKeyState(g_Config.Player->KeyBind) & 0x8000)
					{
						Core::SDK::Pointers::pLocalPlayer->SetHealth(Core::SDK::Pointers::pLocalPlayer->GetMaxHealth());
					}
					...
					// Keybind für armor
					static int KeyMode2 = 2;
					ImGui::Keybind(xorstr("Armor Key"), &g_Config.Player->KeyBind2, &KeyMode2);

					// Überprüfe, ob die gebundene Taste gedrückt wird
					if (GetAsyncKeyState(g_Config.Player->KeyBind2) & 0x8000)
					{
						Core::SDK::Pointers::pLocalPlayer->SetArmor(Core::SDK::Pointers::pLocalPlayer->GetArmor());
					}

					//rest

					if (Custom::Button(xorstr("suicide"), ImVec2(-1, 33), 0))
					{
						Core::SDK::Pointers::pLocalPlayer->SetHealth(g_Config.Player->CurrentHealthValue + 0.f);
					}

					if (Custom::Button(xorstr("Revive"), ImVec2(-1, 33), 0))
					{
						Core::SDK::Pointers::pLocalPlayer->SetHealth(g_Config.Player->CurrentHealthValue + 100.f);
					}
					//if (Custom::Button(xorstr("Crash"), ImVec2(-1, 33), 0))
					//{
