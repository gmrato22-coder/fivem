...
namespace Core {
	...
	inline void GetOffsets()
	{
		...
		g_Offsets.m_World = Mem.ModBase + MemoryUtils::GetOffsetsFromPattern(
			Mem.ProcHandle, Mem.ModBase,
			"48 8B 05 ? ? ? ? 33 D2 48 8B 40 08 8A CA 48 85 C0 74 16 48 8B",
			true
		);

		g_Offsets.m_ReplayInterFace = Mem.ModBase + MemoryUtils::GetOffsetsFromPattern(
			Mem.ProcHandle, Mem.ModBase,
			"48 8D 0D ? ? ? ? 48 ? ? E8 ? ? ? ? 48 8D 0D ? ? ? ? 8A D8 E8 ? ? ? ? 84 DB 75 13 48 8D 0D ? ? ? ? 48 8B D7 E8 ? ? ? ? 84 C0 74 BC 8B 8F",
			false
		);

		g_Offsets.m_ViewPort = Mem.ModBase + MemoryUtils::GetOffsetsFromPattern(
			Mem.ProcHandle, Mem.ModBase,
			"48 8B 15 ? ? ? ? 48 8D 2D ? ? ? ? 48 8B CD",
			true
		);

		g_Offsets.m_BlipList = Mem.ModBase + MemoryUtils::GetOffsetsFromPattern(
			Mem.ProcHandle, Mem.ModBase,
			"4C 8D 05 ? ? ? ? 0F B7 C1",
			false
		);

		g_Offsets.m_CamGameplayDirector = Mem.ModBase + MemoryUtils::GetOffsetsFromPattern(
			Mem.ProcHandle, Mem.ModBase,
		...
		else if (g_Offsets.CurrentBuild == 3258)
		{
			...
			/*FiveM build : 3570
				Base address : 0x7ff699930000
				World : (RIP - relative) : 0x00000000025EC580
				ReplayInterFace : (RIP - relative) : 0x0000000001FB0418
				ViewPort : (RIP - relative) : 0x0000000002058BA0
				BlipList : (RIP - relative) : 0x0000000002061870
				CamGameplayDirector : (RIP - relative) : 0x0000000002059A48
				Camera : (RIP - relative) : 0x0000000002059A48
				Bullet instance : (RIP - relative) : 0x0000000042F5EE67*/
			...
			g_Offsets.m_WeaponManager = 0x10B8;
			g_Offsets.m_LastVehicle = 0x0D10;
			g_Offsets.m_PlayerInfo = 0x10A8;
			g_Offsets.m_PlayerId = 0xE8;
			g_Offsets.m_FragInst = 0x1430;
