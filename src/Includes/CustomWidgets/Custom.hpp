...
namespace Custom {
	...
	inline void LoadingScreen(float duration, const char* text)
	{
		...
		if (elapsed < duration)
			alpha = ImClamp(alpha + ImGui::GetIO().DeltaTime * 2.5f, 0.0f, 1.0f);
		else
			alpha = ImClamp(alpha - ImGui::GetIO().DeltaTime * 3.0f, 0.0f, 1.0f);

		ImDrawList* draw = ImGui::GetForegroundDrawList();
		ImVec2 size = ImGui::GetIO().DisplaySize;

		ImVec4 bg = g_Col.BackgroundCol;
		draw->AddRectFilled({ 0, 0 }, size,
			ImGui::GetColorU32(ImVec4(bg.x, bg.y, bg.z, alpha * 0.95f)));

		ImGui::PushFont(g_Variables.m_FontSecundary);
		ImVec2 textSize = ImGui::CalcTextSize(text);
		ImVec2 textPos = ImVec2((size.x - textSize.x) * 0.5f, (size.y - textSize.y) * 0.5f - 10);
		draw->AddText(textPos,
			ImGui::GetColorU32(ImVec4(g_Col.PrimaryText.x, g_Col.PrimaryText.y, g_Col.PrimaryText.z, alpha)), text);
		ImGui::PopFont();

		float radius = 14.0f;
		int segments = 32;
		float thick = 2.5f;
		float start = ImGui::GetTime() * 4.0f;
		ImVec2 center = { size.x / 2, size.y / 2 + 25 };

		for (int i = 0; i < segments; i++)
		{
			float a = start + (i * IM_PI * 2.0f / segments);
			float b = start + ((i + 1) * IM_PI * 2.0f / segments);
			float t = (float)i / (float)segments;
			draw->AddLine(
				{ center.x + cosf(a) * radius, center.y + sinf(a) * radius },
				{ center.x + cosf(b) * radius, center.y + sinf(b) * radius },
				ImGui::GetColorU32(ImVec4(g_Col.Base.x, g_Col.Base.y, g_Col.Base.z, t * alpha)),
				thick);
		}

		if (elapsed >= duration && alpha <= 0.05f)
			active = false;
	}
	...
	inline ImVec4 MulA(ImVec4 c, float a) { c.w *= a; return c; }
	...
	{
		struct St { float hov = 0.f; float act = 0.f; float pulse = 0.f; float underline = 0.f; };
		...
		const ImVec2 size(134, 34);
		const ImRect r(pos, pos + size);

		ImGui::ItemSize(r, s.FramePadding.y);
		if (!ImGui::ItemAdd(r, id)) return false;

		bool hov, held;
		bool pressed = ImGui::ButtonBehavior(r, id, &hov, &held);

		static std::map<ImGuiID, St> an;
		St& st = an[id];
		st.hov = ImLerp(st.hov, hov ? 1.f : 0.f, g.IO.DeltaTime * 12.f);
		st.act = ImLerp(st.act, active ? 1.f : 0.f, g.IO.DeltaTime * 10.f);
		st.pulse = ImMax(0.f, st.pulse - g.IO.DeltaTime * 8.f);
		if (pressed) st.pulse = 1.f;
		const float targetUnderline = active ? 1.f : (hov ? 0.55f : 0.f);
		st.underline = ImLerp(st.underline, targetUnderline, g.IO.DeltaTime * 10.f);

		const float skew = 10.f;
		ImVec2 p0(r.Min.x, r.Min.y);
		ImVec2 p1(r.Max.x, r.Min.y);
		ImVec2 p2(r.Max.x - skew, r.Max.y);
		ImVec2 p3(r.Min.x, r.Max.y);

		const ImVec4 base = g_Col.Base;
		const float fillA = 0.10f + 0.14f * st.hov + 0.32f * st.act;
		const float brdA = 0.22f + 0.50f * st.act;
		const float pulseA = 0.28f * st.pulse;

		const ImU32 fillCol = ImGui::GetColorU32(ImVec4(base.x, base.y, base.z, fillA));
		const ImU32 borderCol = ImGui::GetColorU32(ImVec4(base.x, base.y, base.z, brdA));
		const ImU32 pulseCol = ImGui::GetColorU32(ImVec4(base.x, base.y, base.z, pulseA));
		const ImU32 lineCol = ImGui::GetColorU32(ImVec4(base.x, base.y, base.z, 0.95f));

		const ImU32 iconCol = ImGui::GetColorU32(active
			? g_Col.PrimaryText
			: ImVec4(ImColor(180 + (int)(40 * st.hov), 180 + (int)(40 * st.hov), 180 + (int)(40 * st.hov))));
		const ImU32 textCol = ImGui::GetColorU32(active ? g_Col.PrimaryText : ImVec4(ImColor(205, 205, 205)));

		w->DrawList->AddQuadFilled(p0, p1, p2, p3, fillCol);
		w->DrawList->AddQuad(p0, p1, p2, p3, borderCol, 1.2f);

		if (st.pulse > 0.f)
			w->DrawList->AddQuad(p0, p1, p2, p3, pulseCol, 3.0f);

		const float pad = 10.f;
		const float uW = (r.GetWidth() - pad * 2.f) * st.underline;
		if (uW > 0.5f)
			w->DrawList->AddRectFilled(ImVec2(r.Min.x + pad, r.Max.y - 3.0f),
				ImVec2(r.Min.x + pad + uW, r.Max.y - 1.0f),
				lineCol, 1.5f);

		ImVec2 isz = Utils::CalcTextSize(g_Variables.FontAwesomeSolid, g_Variables.FontAwesomeSolid->FontSize, icon);
		ImVec2 tsz = Utils::CalcTextSize(g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, label);
		ImVec2 ip = ImVec2(r.Min.x + 12, r.Min.y + (r.GetHeight() - isz.y) * 0.5f - 0.5f);
		ImVec2 tp = ImVec2(ip.x + isz.x + 8, r.Min.y + (r.GetHeight() - tsz.y) * 0.5f - 0.5f);

		w->DrawList->AddText(g_Variables.FontAwesomeSolid, g_Variables.FontAwesomeSolid->FontSize, ip, iconCol, icon);
		w->DrawList->AddText(g_Variables.m_FontNormal, g_Variables.m_FontNormal->FontSize, tp, textCol, label);
	...
	inline bool SubTab(const char* label, bool active) {
		struct St { float hov; };
		ImGuiWindow* w = GetCurrentWindow(); if (w->SkipItems) return false;
		ImGuiContext& g = *GImGui; const ImGuiStyle& s = g.Style; ImGuiID id = w->GetID(label);
		ImVec2 pos = w->DC.CursorPos; ImVec2 ts = Utils::CalcTextSize(g_Variables.m_FontSecundary, g_Variables.m_FontSecundary->FontSize, label);
		ImRect r(pos, pos + ImVec2(ts.x + 22, 28)); ItemSize(r, s.FramePadding.y); if (!ItemAdd(r, id)) return false;
		bool hov, held; bool pressed = ButtonBehavior(r, id, &hov, &held);
		static std::map<ImGuiID, St> an; auto it = an.find(id); if (it == an.end()) { an.insert({ id,St() }); it = an.find(id); }
		it->second.hov = ImLerp(it->second.hov, hov ? 1.f : 0.f, g.IO.DeltaTime * 10.f);
		ImU32 tx = GetColorU32(active ? ImVec4(ImColor(235, 235, 235)) : ImVec4(ImColor(180 + (int)(40 * it->second.hov), 180 + (int)(40 * it->second.hov), 180 + (int)(40 * it->second.hov))));
		w->DrawList->AddText(g_Variables.m_FontSecundary, g_Variables.m_FontSecundary->FontSize, ImVec2(r.Min.x + 11, r.Min.y + (r.GetHeight() - ts.y) * 0.5f - 1), tx, label);
		if (active || it->second.hov > 0.01f) w->DrawList->AddLine(ImVec2(r.Min.x + 9, r.Max.y - 3), ImVec2(r.Min.x + 9 + ts.x + 4, r.Max.y - 3), GetColorU32(MulA(g_Col.Base, active ? 0.9f : 0.5f * it->second.hov)), 2.f);
		IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
		return pressed;
	}
	...
	inline bool Button(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool bToolTip = false, const char* ToolTipMsg = "", const char* ToolTipIcon = "")
	{
		...
		// Render
		const ImU32 inside_solid_col = GetColorU32(g_Col.ChildCol);
		const ImU32 outside_solid_col = GetColorU32(g_Col.ChildBorderCol);

		ImVec4 base_with_alpha = g_Col.Base;
		base_with_alpha.w = ImClamp((int)it_anim->second.closing_alpha, 0, 255) / 255.0f;
		const ImU32 inside_hover_col = GetColorU32(base_with_alpha);
