...
namespace Gui
{
	...
	inline LRESULT CALLBACK KeyboardCallBack(int nCode, WPARAM wParam, LPARAM lParam)
	{
		if (nCode >= 0)
		{
			...
			// Ignore Alt and Alt+Tab to avoid Default Beep on the overlay window
			if (pKeyboard->vkCode == VK_MENU)
				return CallNextHookEx(hKeyboardHook, nCode, wParam, lParam);
			if (pKeyboard->vkCode == VK_TAB && (GetAsyncKeyState(VK_MENU) & 0x8000))
				return CallNextHookEx(hKeyboardHook, nCode, wParam, lParam);

			// Only forward keys to the overlay when it is active (menu open and overlay focused/top)
			if (GetForegroundWindow() == g_Variables.g_hCheatWindow && g_MenuInfo.IsOpen)
	...
	LRESULT CALLBACK Overlay::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		case WM_SYSKEYDOWN:
		case WM_SYSKEYUP:
			// Swallow system key messages (ALT/ALT+TAB) to prevent Default Beep
			return 0;
		case WM_DESTROY:
			::PostQuitMessage(0);
			return 0;
